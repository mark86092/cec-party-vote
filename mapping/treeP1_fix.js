...
//候選人得票數
var root_P1 = module.exports.root_P1 = new Mapping('總統副總統', "http://www.cec.gov.tw/zh_TW/P1/"+orderBy+"000000000000000.html");
//aux_1 = insFld(foldersTree, gFld(menuLevel_1, "../P1/"+orderBy+"000000000000000.html"));
for (x=0;x<firAreaName.length;x++){
	i=arrOrder[x];
	aux_arr_1[i] = root_P1.addNode(new Mapping(firAreaName[i], "http://www.cec.gov.tw/zh_TW/P1/"+orderBy+firAreaID[i]+'.html'));
	//aux_arr_1[i]=insFld(aux_1, gFld(firAreaName[i], '../P1/'+orderBy+firAreaID[i]+'.html'));
	for (j=1;j<thrAreaName[i].length;j++){
		aux_arr_1[i].addNode(new Mapping(thrAreaName[i][j], "http://www.cec.gov.tw/zh_TW/P1/"+orderBy+thrAreaID[i][j]+'00000000'+'.html'));
		//insDoc(aux_arr_1[i], gLnk('S', thrAreaName[i][j], '../P1/'+orderBy+thrAreaID[i][j]+'00000000'+'.html'));
	}
}
/*
//候選人簡歷
aux_2=insDoc(foldersTree,gLnk("S",menuLevel_2,'../CP/'+'000000000000000'+'.html'));

//選舉概況
aux_3 = insFld(foldersTree, gFld(menuLevel_3, "../FP1/000000000000000.html"));
var aux_arr_3=new Array();
for (x=0;x<firAreaName.length;x++){
	i=arrOrder[x];
	aux_arr_3[i]=insFld(aux_3, gFld(firAreaName[i], '../FP1/'+firAreaID[i]+'.html'));
	for (j=1;j<thrAreaName[i].length;j++){
		insDoc(aux_arr_3[i], gLnk('S', thrAreaName[i][j], '../FP1/'+thrAreaID[i][j]+'00000000'+'.html'));
	}
}
*/
