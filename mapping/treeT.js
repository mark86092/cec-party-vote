		...
		for(k=0;k<=secAreaNumber[i][j];k++){
			...
			for(l=0;l<thrAreaName[i][j][k].length;l++){
				if(thrAreaName[i][j][k][l]!="" && thrAreaName[i][j][k][l]!=undefined && thrAreaID[i][j][k][l]!=undefined){
					//縣市
					if(k==0 && l==0){
						aux_arr_1[i] = insFld(area_t1, gFld(thrAreaName[i][j][k][l], ""));
					}
					//選區 
					if(k!=0 && l==0){
						aux_arr_1_a[i][j] = insFld( aux_arr_1[i], gFld(thrAreaName[i][j][k][l], "../T1/"+orderBy+thrAreaID[i][j][k][l]+"0000"+"0000"+".html"));
					}
					//鄉鎮
					else  
					{
